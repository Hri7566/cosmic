    ...
    new Command(
        ...
        async (msg, cl) => {
            ...
            let listener2 = function (msg) {
                if (
                    msg.a.toLowerCase().endsWith("who") ||
                    msg.a.toLowerCase().endsWith("who?")
                ) {
                    cl.sendChat(joke.a);
                    (cl as CosmicClientMPP).client.off("a", listener2);
                    knockKnocking = false;
                }
            };
            ...
            let listener1 = function (msg) {
                if (
                    msg.a.toLowerCase() == `who's there` ||
                    msg.a.toLowerCase() == "whos there" ||
                    msg.a.toLowerCase() == `who's there?` ||
                    msg.a.toLowerCase() == "whos there?"
                ) {
                    cl.sendChat(joke.q);
                    (cl as CosmicClientMPP).client.off("a", listener1);
                    (cl as CosmicClientMPP).client.on("a", listener2);
                }
            };
            ...
            joke =
                knockKnockJokes[
                    Math.floor(Math.random() * knockKnockJokes.length)
                ];
            (cl as CosmicClientMPP).client.on("a", listener1);
        }
    )
);
