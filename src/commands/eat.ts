    ...
    new Command(
        ...
        async (msg, cl) => {
            ...
            try {
                ...
                if (arg_to_check !== "1") {
                    ...
                    if (!isNaN(amount_to_remove)) {
                        argcat = argcat
                            .substring(0, argcat.length - arg_to_check.length)
                            .trim();
                    }
                }
            } finally {
            ...
            for (const it of inv.items) {
                if (
                    it.displayName.toLowerCase().includes(argcat.toLowerCase())
                ) {
                    mod_it = it;
                    break;
                }
            ...
            if (mod_it) {
                const res1 = await CosmicData.removeOneItem(
                    msg.sender._id,
                    mod_it.id,
                    amount_to_remove || 1
                );
                ...
                if (
                    mod_it.id.startsWith("cake") ||
                    mod_it.id.startsWith("cupcake")
                ) {
                    let bal_add =
                        (mod_it.value || CosmicCakeFactory.DEFAULT_CAKE_VALUE) *
                        amount_to_remove;
                    ...
                    return `You ate ${
                        amount_to_remove == 1
                            ? /^[aeiou]/.test(mod_it.displayName)
                                ? "an"
                                : "a"
                            : amount_to_remove + " of"
                    } ${mod_it.displayName} and got ${CosmicData.formatBalance(
                        bal_add
                    )}.`;
                }
